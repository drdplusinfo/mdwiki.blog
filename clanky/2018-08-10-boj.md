...
Když si vezmeme původní pravidla, tak body *Boje* ovlivňují nějaké ty [základní vlastnosti](http://pph.drdplus.loc:88/#tabulka_boje), k tomu hod 2k6± a pak spousta úprav jako [dovednost se zbraní](http://pph.drdplus.loc:88/#boj_se_zbrani), [únava](http://pph.drdplus.loc:88/#postih_za_unavu) a [zranění](http://pph.drdplus.loc:88/#postih_za_zraneni), [terén](http://bojovnik.drdplus.loc:88/#boj_v_terenu_i) a tak.
...
Jenže krom bonusů jsou tu stále i postihy. Postih za [nedostatečnou sílu](http://pph.drdplus.loc:88/#tabulka_postihu_ke_zbrani_za_chybejici_silu) (kterou chceme zjednodušit na -1 ke všemu k boji za každý chybějící bod *Síly*), [postih za únavu](https://pph.drdplus.info/#postih_za_unavu) a [zranění](https://pph.drdplus.info/#postih_za_zraneni) a určitě by se ještě [něco našlo](https://pph.drdplus.info/#postizeni).
...
- za každý chybějící, nedostatečný bod *Boje* dostanše postih -1 ke všemu, co v akci provádíš
...
Ovšem pokud máš v ruce třeba [dlouhou dýku](https://pph.drdplus.info/#tabulka_zbrani_jednorucni_zbrane) s délkou 1, tak... tak nic, protože to není délka v metrech, ale v "půlmetrech", takže ti dáme do ruky radši kopí s délkou čtyři, což jsou dva metry a najednou bys lučištníka po doběhnutí už devíti metrů zase mohl poškádlit (z devíti metrů už se zbraní s dosahem dvou metrů dosáhneš na lučištníkův jedenáctý metr), byť s postihem k Útoku -6, jelikož ti došly body *Boje*.

Tím jsme se dostali k délce zbraně, která podle původních pravidel [přidává bonus k *Boji*](http://pph.drdplus.loc:88/#delka_zbrane). Inu, v původních pravidlech proč ne, útok podle nich máš stejně jeden za kolo, takže útočit budeš pouze tou zbraní, kterou jsi na začátku popadnul.
...
V Pravidlech pro hráče má [každé povolání jiný základ *Boje*](https://pph.drdplus.info/#tabulka_boje).
...
- za *Obratnost*?
  - **ano**, je to souhrn instinktů, schopnost reagovat na nenadálé situace
    - ještě přemýšlíme o *Smyslech* jako základu boje, což by ale silně narušil zloděj se svými [Zdokonalenými smysly](http://zlodej.drdplus.loc:88/#zdokonalene_smysly)
- za *Zručnost*?
...
- body *Boje* mohu mezi akcemi přesouvat, ale pouze na začátku každé *vlny akcí*
    - za každý bod *Boje* navíc, kterou na akci použiju, dostanu k této akci bonus +1 (ať už je to cokoliv, útok, obrana, kouzlení, zastrašování, [hraní bezbranného](http://zlodej.drdplus.loc:88/#neskodny_vzhled)...)
...
- "zadarmo" se můžu kdykoli dát do běhu plnou [Rychlostí](http://pph.drdplus.loc:88/#rychlost_pohybu) (útěk, pronásledování), ale u takového pohybu mám -6 ke všem činnostem od "teď" do konce tohoto kola (ztratím kontakt s bojem)
